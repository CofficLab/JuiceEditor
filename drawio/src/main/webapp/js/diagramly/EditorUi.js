...
;(function () {
   ...
   */
  EditorUi.enableDrafts = !mxClient.IS_CHROMEAPP && isLocalStorage && urlParams['drafts'] != '0'
  ...
  EditorUi.isVisioFilename = function (filename) {
    return /(\.v(dx|sdx?))($|\?)/i.test(filename) || /(\.vs(x|sx?))($|\?)/i.test(filename)
  }
      ...
      try {
        ...
        if (idx >= 0) {
          ...
          var node = this.editor.extractGraphModel(
            doc.documentElement,
            this.pages != null || this.diagramContainer.style.visibility == 'hidden'
          )
  ...
  EditorUi.prototype.validateFileData = function (data) {
    if (data != null && data.length > 0) {
      ...
      if (index >= 0) {
        ...
        data =
          data.slice(0, index) + replaceString + data.slice(index + replaceStrLen - 1, data.length)
      }
    ...
    if (node == null) {
      ...
      if (!compact) {
        ...
        fileNode.setAttribute(
          'agent',
          navigator.userAgent != null ? navigator.userAgent : navigator.appVersion
        )
      ...
      // Maps the XML data to the content attribute in the SVG node
        ...
        if (
          file != null &&
          (file.getMode() == App.MODE_DEVICE || file.getMode() == App.MODE_BROWSER)
        ) {
          url = null
        }
        ...
        for (var i = 0; i < this.pages.length; i++) {
          ...
          if (page != this.currentPage) {
            if (page.needsUpdate) {
              ...
              this.editor.graph.saveViewState(page.viewState, temp, null, resolveReferences)
            ...
            if (cellDiffs != null) {
              for (var cellId in cellDiffs) {
                if (cellDiffs[cellId].value != null) {
                  cellDiffs[cellId].value = '[' + cellDiffs[cellId].value.length + ']'
                }

                if (cellDiffs[cellId].xmlValue != null) {
                  cellDiffs[cellId].xmlValue = '[' + cellDiffs[cellId].xmlValue.length + ']'
                }

                if (cellDiffs[cellId].style != null) {
                  cellDiffs[cellId].style = '[' + cellDiffs[cellId].style.length + ']'
                }
  ...
  EditorUi.prototype.getFileData = function (
    ...
    node =
      node != null
        ? node
        : this.getXmlFileData(ignoreSelection, currentPage, uncompressed, resolveReferences)
  ...
  EditorUi.prototype.downloadFile = function (
    ...
    try {
      ignoreSelection =
        ignoreSelection != null ? ignoreSelection : this.editor.graph.isSelectionEmpty()
  ...
  EditorUi.prototype.downloadRequestBuilder = function (
    ...
    if (
      !EditorUi.isElectronApp &&
      (bounds.width * bounds.height > MAX_AREA || data.length > MAX_REQUEST_SIZE)
    ) {
  ...
  EditorUi.prototype.loadDescriptor = function (desc, success, error) {
    ...
    var loadData = mxUtils.bind(this, function (data) {
      ...
      var tempFile = new LocalFile(
        this,
        xml,
        urlParams['title'] != null ? decodeURIComponent(urlParams['title']) : this.defaultFilename,
        true
      )
      ...
      if (desc.update != null) {
        ...
        var doUpdate = mxUtils.bind(this, function () {
          ...
          mxUtils.post(
            desc.update,
            'xml=' + encodeURIComponent(mxUtils.getXml(this.editor.getGraphXml())),
    ...
    try {
      if (file != null) {
        ...
        if (
          ...
          var evt = {
            category: 'WARN-CLOSE-MODIFIED-FILE-' + file.getHash(),
            action:
              (discarded ? 'DISCARDED' : 'WARNED') +
              (file.savingFile ? 'saving' : '') +
              (file.savingFile && file.savingFileTime != null
                ? '_' + Math.round((Date.now() - file.savingFileTime.getTime()) / 1000)
                : '') +
    ...
    if (file != null) {
      try {
        ...
        if (EditorUi.enableLogging && !this.isOffline()) {
          try {
            EditorUi.logEvent({
              category: 'ERROR-LOAD-FILE-' + (file != null ? file.getHash() : 'none'),
    ...
    if (
      ...
      if (obj.attributes != null) {
        ...
        for (var i = 0; i < obj.attributes.length; i++) {
          ...
          if (value != null) {
            hash =
              hash ^
              (this.hashValue(key, replacer, details) + this.hashValue(value, replacer, details))
          }
        }
      }
      ...
      if (obj.childNodes != null) {
        for (var i = 0; i < obj.childNodes.length; i++) {
          hash = ((hash << 5) - hash + this.hashValue(obj.childNodes[i], replacer, details)) << 0
        }
      }
  ...
  EditorUi.prototype.libraryLoaded = function (file, images, optionalTitle, expand) {
    ...
    if (file.constructor != StorageLibrary || file.title != '.scratchpad') {
      ...
      library = { file: file, images: images, title: optionalTitle, expand: expand }
          ...
          mxUtils.bind(this, function (evt) {
            ...
            if (evt.dataTransfer.files.length > 0) {
              this.importFiles(
                ...
                0,
                this.maxImageSize,
                mxUtils.bind(this, function (data, mimeType, x, y, w, h, img, doneFn, file) {
                  if (data != null && mimeType.substring(0, 6) == 'image/') {
                    ...
                    addCells(
                      ...
                      mxEvent.isAltDown(evt)
                        ? null
                        : img.substring(0, img.lastIndexOf('.')).replace(/_/g, ' ')
                    )
                                ...
                                this.handleError(
                                  {
                                    message: mxResources.get(
                                      xhr.status == 413 ? 'drawingTooLarge' : 'invalidOrMissingFile'
                                    )
                                  },
  ...
  EditorUi.prototype.handleError = function (
    ...
    var resume =
      this.spinner != null && this.spinner.pause != null ? this.spinner.pause() : function () {}
    ...
    if (resp != null && (urlParams['test'] == '1' || resp.stack != null) && resp.message != null) {
      try {
        if (!disableLogging) {
          EditorUi.logError(
            'Caught: ' + (resp.message == '' && resp.name != null) ? resp.name : resp.message,
            resp.filename,
            resp.lineNumber,
            resp.columnNumber,
            resp,
        ...
        if ((urlParams['test'] == '1' || disableLogging) && window.console != null) {
  ...
  EditorUi.prototype.showBanner = function (
    ...
    if (
      !this.bannerShowing &&
      !this['hideBanner' + id] &&
      (!isLocalStorage || mxSettings.settings == null || mxSettings.settings['close' + id] == null)
    ) {
      ...
      if (!small) {
        ...
        logo.style.cssText =
          'margin-top:-4px;margin-left:8px;' + 'margin-right:12px;width:26px;height:26px;'
  ...
  EditorUi.prototype.saveCanvas = function (canvas, xml, format, ignorePageName, dpi) {
    ...
    var filename = this.getBaseFilename(ignorePageName) + (xml != null ? '.drawio' : '') + '.' + ext
  ...
  EditorUi.prototype.doSaveLocalFile = function (
    ...
    // Older versions of IE (binary not supported)
      ...
      var useDownload =
        (navigator.userAgent == null || navigator.userAgent.indexOf('PaleMoon/') < 0) &&
      ...
      if (useDownload || this.isOffline()) {
        a.href = URL.createObjectURL(
          base64Encoded ? this.base64ToBlob(data, mimeType) : new Blob([data], { type: mimeType })
        )
  ...
  EditorUi.prototype.saveLocalFile = function (
    ...
    allowTab =
      allowTab != null ? allowTab : format != 'vsdx' && (!mxClient.IS_IOS || !navigator.standalone)
  ...
  EditorUi.prototype.saveRequest = function (
    ...
    var saveFunction = mxUtils.bind(this, function (newTitle, mode, input, folderId) {
      if (mode == '_blank' || (newTitle != null && newTitle.length > 0)) {
        var base64 =
          mode == App.MODE_DEVICE || mode == 'download' || mode == null || mode == '_blank'
            ? '0'
            : '1'
    ...
    if (this.spinner.spin(document.body, mxResources.get('export'))) {
      try {
        ...
        var svgRoot = this.editor.graph.getSvg(
          ...
          null,
          null,
          linkTarget == 'blank' ? '_blank' : linkTarget == 'self' ? '_top' : null,
          null,
          !embedFonts,
          theme,
          exportType
  ...
  EditorUi.prototype.addLinkSection = function (div, showFrameOption) {
    ...
    if (showFrameOption) {
      ...
      mxUtils.write(
        frameOption,
        mxResources.get('openInThisWindow') + ' (' + mxResources.get('iframe') + ')'
      )
  ...
  EditorUi.prototype.createUrlParameters = function (
    ...
    if (lightbox) {
      ...
      if (linkColor != null && linkColor != mxConstants.NONE) {
        params.push(
          'highlight=' + (linkColor.charAt(0) == '#' ? linkColor.substring(1) : linkColor)
        )
      }
  ...
  EditorUi.prototype.showPublishLinkDialog = function (
    ...
    if (file != null && file.constructor == window.DriveFile && !hideShare) {
            ...
            this.getPublicUrl(
              ...
              mxUtils.bind(this, function (url) {
                ...
                var dlg = new ErrorDialog(
                  this,
                  null,
                  mxResources.get(url != null ? 'diagramIsPublic' : 'diagramIsNotPublic'),
                  mxResources.get('ok')
                )
      ...
      helpLink =
        helpLink != null ? helpLink : 'https://www.drawio.com/doc/faq/publish-diagram-as-link'
    }
    ...
    var dlg = new CustomDialog(
      ...
      mxUtils.bind(this, function () {
        ...
        callback(
          ...
          exportSelect.value,
          txtSettingsSelect.value == 'embedFonts',
  ...
  EditorUi.prototype.createEmbedImage = function (
    ...
    if (this.isExportToCanvas()) {
      ...
      if (bounds.width * bounds.height <= MAX_AREA && data.length <= MAX_REQUEST_SIZE) {
        ...
        if (retina) {
          size = '&w=' + Math.round(2 * bounds.width) + '&h=' + Math.round(2 * bounds.height)
        }

        var embed = lightbox ? '1' : '0'
        var req = new mxXmlRequest(
          EXPORT_URL,
          'format=png' + '&base64=1&embedXml=' + embed + size + '&xml=' + encodeURIComponent(data)
        )
  ...
  EditorUi.prototype.createEmbedSvg = function (fit, shadow, image, lightbox, edit, layers, fn) {
    ...
    if (links != null) {
      for (var i = 0; i < links.length; i++) {
        ...
        if (href != null && href.charAt(0) == '#' && links[i].getAttribute('target') == '_blank') {
    ...
    try {
      ...
      if (optionalData != null && optionalData.length > 0) {
        ...
        this.decodeNodeIntoGraph(
          this.editor.extractGraphModel(mxUtils.parseXml(optionalData).documentElement, true),
          graph
        )
            ...
            if (diagrams.length == 1 && !addNewPage) {
              ...
              if (this.currentPage != null) {
                ...
                if (this.isBlankFile()) {
                  ...
                  if (name != null && name != '') {
                    this.editor.graph.model.execute(new RenamePage(this, this.currentPage, name))
                  }
                }
              }
          ...
          if (this.doImportVisio) {
            ...
            if (remote) {
              if (VSD_CONVERT_URL != null && !this.isOffline()) {
                ...
                xhr.open(
                  'POST',
                  VSD_CONVERT_URL + (/(\.vss|\.vsx)$/.test(filename) ? '?stencil=1' : '')
                )
  ...
  EditorUi.prototype.createMermaidXml = function (input, config, data, w, h) {
    ...
    var cell = graph.insertVertex(
      ...
      w,
      h,
      'shape=image;noLabel=1;verticalAlign=top;' + 'imageAspect=1;image=' + data + ';'
    )
    graph.setAttributeForCell(
      cell,
      'mermaidData',
  ...
  EditorUi.prototype.generateMermaidImage = function (
      ...
      try {
        ...
        var renderCallback = mxUtils.bind(this, function (svg) {
          try {
            ...
            if (mxClient.IS_IE || mxClient.IS_IE11) {
              svg = svg
                .replace(/ xmlns:\S*="http:\/\/www.w3.org\/XML\/1998\/namespace"/g, '')
    ...
    if (typeof mermaid === 'undefined' && !this.loadingMermaid && !this.isOffline(true)) {
      ...
      if (urlParams['dev'] == '1') {
        ...
        mxscript('js/mermaid/mermaid.min.js', delayed, null, null, null, onerror)
      } else {
        mxscript('js/extensions.min.js', delayed, null, null, null, onerror)
      }
    } else {
      window.setTimeout(delayed, 0)
    }
  ...
  EditorUi.prototype.importFiles = function (
    ...
    var doImportFiles = mxUtils.bind(this, function () {
      ...
      fn =
        fn != null
          ? fn
          : mxUtils.bind(this, function (data, mimeType, x, y, w, h, filename, done, file) {
              try {
                                    ...
                                    barrier(
                                      ...
                                      mxUtils.bind(this, function () {
                                        ...
                                        if (data2 != null && data2.length < maxBytes) {
                                          var s =
                                            !resizeImages ||
                                            !this.isResampleImageSize(file.size, resampleThreshold)
                                              ? 1
  ...
  EditorUi.prototype.parseFileData = function (data, fn, filename) {
    ...
    xhr.send(
      'format=xml&filename=' + encodeURIComponent(filename) + '&data=' + encodeURIComponent(data)
    )

    try {
      EditorUi.logEvent({ category: 'GLIFFY-IMPORT-FILE', action: 'size_' + file.size })
    } catch (e) {
      // ignore
    }
  }
  ...
  EditorUi.prototype.init = function () {
    ...
    if (Graph.touchStyle) {
      graph.panningHandler.isPanningTrigger = function (me) {
        ...
        return (
          (me.getState() == null && !mxEvent.isMouseEvent(evt) && !graph.freehand.isDrawing()) ||
                ...
                function (data, w, h) {
                  ...
                  try {
                    ...
                    graph.setAttributeForCell(
                      cell,
                      'mermaidData',
                      JSON.stringify({ data: text, config: obj.config }, null, 2)
                    )
                  } finally {
                    graph.getModel().endUpdate()
                  }
    ...
    if (this.menus != null) {
      ...
      this.menus.addPopupMenuEditItems = function (menu, cell, evt) {
        if (ui.editor.graph.isSelectionEmpty()) {
          ...
          this.addMenuItems(menu, ['-', 'cut', 'copy', 'copyAsImage', 'duplicate', '-'], null, evt)
    ...
    ui.actions.get('print').funct = function () {
      ui.showDialog(
        ...
        360,
        ui.pages != null && ui.pages.length > 1 ? 470 : 390,
        true,
        true
      )
    }
    ...
    this.addListener(
      ...
      mxUtils.bind(this, function () {
        var file = this.getCurrentFile()
        var filename =
          file != null && file.getTitle() != null ? file.getTitle() : this.defaultFilename
      ...
      if (footer != null) {
        ...
        mxEvent.addListener(
          ...
          mxUtils.bind(this, function () {
            this.editor.graph
              .getModel()
              .setStyle(this.editor.graph.getSelectionCell(), this.styleInput.value)
          })
        )
          ...
          if (graph.isEnabled()) {
            ...
            if (files.length > 0) {
              if (urlParams['embed'] != '1' && mxEvent.isShiftDown(evt)) {
                ...
                if (this.isBlankFile() && !this.canUndo() && this.getCurrentFile() != null) {
              ...
              if (data != null) {
                ...
                graph.setSelectionCells(
                  this.insertTextAt(evt.dataTransfer.getData('text/plain'), x, y, true)
                )
              }
            }
          }
    ...
    mxWindow.prototype.fit = function () {
      if (!Editor.inlineFullscreen && ui.embedViewport != null) {
        ...
        this.div.style.height =
          Math.min(ui.embedViewport.height, parseInt(this.div.style.height)) + 'px'
        this.div.style.width =
          Math.min(ui.embedViewport.width, parseInt(this.div.style.width)) + 'px'
      } else {
    ...
    if (Editor.currentTheme != 'atlas') {
      if (
        ...
        if (darkMode || urlParams['dark'] == '1' || Editor.currentTheme == 'dark') {
  ...
  EditorUi.prototype.initializeInlineEmbedMode = function () {
    ...
    mxEvent.addGestureListeners(
      ...
      mxUtils.bind(this, function (evt) {
        if (mxEvent.getSource(evt) == this.diagramContainer.parentNode) {
          this.embedExitPoint = new mxPoint(mxEvent.getClientX(evt), mxEvent.getClientY(evt))
    ...
    mxEvent.addGestureListeners(
      ...
      mxUtils.bind(this, function (evt) {
        ...
        if (x0 != null) {
          this.diagramContainer.style.width = Math.max(20, w0 + mxEvent.getClientX(evt) - x0) + 'px'
          changed = true
        }
  ...
  EditorUi.prototype.isDefaultTheme = function (theme) {
    return (
      theme == '' || theme == 'dark' || theme == 'default' || theme == 'kennedy' || theme == null
    )
  }
                    ...
                    mxUtils.bind(this, function () {
                      ...
                      if (isLocalStorage && isSimple(value)) {
                        this.setTabContainerVisible(
                          mxSettings.settings.pages != null ? mxSettings.settings.pages : true
                        )
                      }
  ...
  EditorUi.prototype.installStatusMinimizer = function (parent) {
    ...
    var statusChanged = mxUtils.bind(this, function () {
      if (Editor.currentTheme == 'sketch') {
        ...
        parent.style.visibility = empty && this.editor.getStatus() == '' ? 'hidden' : ''
  ...
  EditorUi.prototype.switchTheme = function (value) {
    ...
    if (value == 'sketch') {
      ...
      // Shapes window
      var libs =
        Editor.enableCustomLibraries && (urlParams['embed'] != '1' || urlParams['libraries'] == '1')
  ...
  EditorUi.prototype.isPageMenuVisible = function () {
    return (
      this.pages != null &&
      (urlParams['pages'] != '0' || this.pages.length > 1 || Editor.pagesVisible)
    )
  }
    ...
    if (this.isOffline(true)) {
      ...
      if (file != null) {
        if (file.invalidChecksum) {
          ...
          status = mxResources.get('realtimeCollaboration') + ': ' + mxResources.get('disabled')
          ...
          status =
            ...
            ': ' +
            (err != null && err.message != null ? err.message : mxResources.get('error'))
        }
      }
    }
  ...
  EditorUi.prototype.createMenuItem = function (key, img, ignoreState) {
    ...
    var btn = this.createToolbarButton(
      img,
      mxResources.get(key) +
        (action != null && action.shortcut != null ? ' (' + action.shortcut + ')' : ''),
      fn
    )
  ...
  EditorUi.prototype.toggleFormatPanel = function (visible) {
    ...
    if (wnd != null) {
      wnd.window.setVisible(visible != null ? visible : !this.isFormatPanelVisible())
    } else {
      ...
      if (urlParams['embedInline'] != '1') {
        ...
        if (Editor.currentTheme == 'simple') {
          ...
          var effHsplitPosition = Math.max(
            0,
            Math.min(iw - this.formatWidth, Math.min(this.hsplitPosition, w - this.splitSize - 40))
          )
  ...
  EditorUi.prototype.createShapesWindow = function () {
    if (this.sidebarWindow == null) {
      ...
      var simpleTheme = Editor.currentTheme == 'simple' || Editor.currentTheme == 'sketch'
      ...
      this.sidebarWindow = new WrapperWindow(
        ...
        simpleTheme && urlParams['embedInline'] != '1'
          ? Math.max(30, (this.diagramContainer.parentNode.clientHeight - h) / 2)
          : 56,
        w - 6,
        h - 6,
        mxUtils.bind(this, function (container) {
  ...
  EditorUi.setGraphDarkMode = function (graph, container, darkMode) {
    ...
    graph.defaultPageBackgroundColor =
      urlParams['embedInline'] == '1' ? 'transparent' : darkMode ? Editor.darkColor : '#ffffff'
    ...
    if (Editor.enableCssDarkMode) {
      ...
      if (this.darkStyle != null) {
        ...
        this.createTimeout(
          ...
          mxUtils.bind(this, function (timeout) {
            darkStyle.onerror = mxUtils.bind(this, function (e) {
              if (timeout.clear()) {
                error(
                  new Error(
                    mxResources.get('errorLoadingFile') + ' ' + darkStyle.getAttribute('href')
                  )
                )
              }
            })
          ...
          mxUtils.bind(this, function () {
            error(new Error(mxResources.get('timeout') + ' ' + darkStyle.getAttribute('href')))
          })
        )
      }
    }
  ...
  EditorUi.prototype.setInlineFullscreen = function (value) {
    if (Editor.inlineFullscreen != value) {
      this.diagramContainer.setAttribute('data-scrollState', JSON.stringify(this.saveScrollState()))
  ...
  EditorUi.prototype.inlineSizeChanged = function () {
    ...
    if (Editor.inlineFullscreen) {
      ...
      toolbar.style.top = this.diagramContainer.offsetTop - toolbar.offsetHeight - 4 + 'px'

      picker.style.display = ''
      picker.style.left = this.diagramContainer.offsetLeft - picker.offsetWidth - 4 + 'px'
      ...
      this.bottomResizer.style.left =
        this.diagramContainer.offsetLeft +
        (this.diagramContainer.offsetWidth - this.bottomResizer.offsetWidth) / 2 +
        'px'
      ...
      this.rightResizer.style.top =
        this.diagramContainer.offsetTop +
        (this.diagramContainer.offsetHeight - this.bottomResizer.offsetHeight) / 2 +
        'px'
    }
  ...
  EditorUi.prototype.updateDefaultStyles = function () {
    ...
    if (
      Editor.config == null ||
      (Editor.config.defaultVertexStyle == null && Editor.config.defaultEdgeStyle == null)
    ) {
    ...
    if (Editor.isSettingsEnabled()) {
      ...
      if (this.sidebar != null) {
        if (urlParams['search-shapes'] != null && this.sidebar.searchShapes != null) {
          ...
          if (
            (!this.editor.chromeless || this.editor.editable) &&
            (mxSettings.settings.isNew || parseInt(mxSettings.settings.version || 0) <= 8)
          ) {
    ...
    try {
      if (
        ...
        this.editor.exportToCanvas(
          ...
          null,
          scale != null ? scale : 4,
          this.editor.graph.background == null || this.editor.graph.background == mxConstants.NONE,
          null,
          null,
          null,
          10,
        ...
        mxEvent.addListener(
          ...
          mxUtils.bind(this, function (evt) {
            if (this.editor.graph.isEnabled() || urlParams['embed'] != '1') {
              ...
              if (
                dropElt == null &&
                (!mxClient.IS_IE || (document.documentMode > 10 && document.documentMode < 12))
              ) {
  ...
  EditorUi.prototype.openFileHandle = function (data, name, file, temp, fileHandle) {
    if (name != null && name.length > 0) {
      if ((!this.useCanvasForExport && /(\.png)$/i.test(name)) || /(\.pdf)$/i.test(name)) {
    ...
    if (this.spinner.spin(document.body, mxResources.get('loading'))) {
      for (var i = 0; i < files.length; i++) {
        mxUtils.bind(this, function (file) {
          ...
          if (
            (file.type.substring(0, 5) === 'image' || file.type === 'application/pdf') &&
  ...
  EditorUi.prototype.openLocalFile = function (data, name, temp, fileHandle, desc) {
    ...
    var fn = mxUtils.bind(this, function () {
      ...
      if (name == null && this.getCurrentFile() != null && this.isDiagramEmpty()) {
        ...
        this.fileLoaded(
          new LocalFile(this, data, name || this.defaultFilename, temp, fileHandle, desc)
        )
      }
    })
  ...
  EditorUi.prototype.setGraphEnabled = function (enabled) {
    ...
    this.hsplit.style.display =
      enabled && Editor.currentTheme != 'sketch' && Editor.currentTheme != 'min' ? '' : 'none'
  ...
  EditorUi.prototype.sendEmbeddedSvgExport = function (noExit) {
    try {
      ...
      if (!this.editor.modified) {
        if (!noExit) {
          parent.postMessage(JSON.stringify({ event: 'exit', point: this.embedExitPoint }), '*')
        }
      } else {
        var bg = graph.background
        ...
        this.getEmbeddedSvg(
          this.getFileData(true, null, null, null, null, null, null, null, null, false),
          graph,
          null,
          true,
          mxUtils.bind(this, function (svg) {
      ...
      mxUtils.bind(this, function (evt) {
        ...
        if (urlParams['proto'] == 'json') {
          ...
          try {
            ...
            EditorUi.debug('EditorUi.installMessageHandler', [this], 'evt', [evt], 'data', [data])
          } catch (e) {
            data = null
          }
            ...
            if (data == null) {
              ...
              var dlg = new FilenameDialog(
                ...
                function (value) {
                  if (value != null) {
                    parent.postMessage(
                      JSON.stringify({ event: 'prompt', value: value, message: data }),
                      '*'
                    )
                  } else {
                    parent.postMessage(
              ...
              var dlg = new DraftDialog(
                this,
                mxResources.get('draftFound', [data.name || this.defaultFilename]),
                tmp,
                mxUtils.bind(this, function () {
                  this.hideDialog()
                  parent.postMessage(
                    JSON.stringify({ event: 'draft', result: 'edit', message: data }),
                    '*'
                  )
                }),
                mxUtils.bind(this, function () {
                  this.hideDialog()
                  parent.postMessage(
                    JSON.stringify({ event: 'draft', result: 'discard', message: data }),
                    '*'
                  )
                }),
                ...
                data.ignore
                  ? mxUtils.bind(this, function () {
                      this.hideDialog()
                      parent.postMessage(
                        JSON.stringify({ event: 'draft', result: 'ignore', message: data }),
                        '*'
                      )
                    })
                  : null
              ...
              try {
                ...
                parent.postMessage(
                  JSON.stringify({ event: 'draft', error: e.toString(), message: data }),
                  '*'
                )
              }
              ...
              var dlg = new NewDialog(
                ...
                enableRecentDocs
                  ...
                  : null,
                enableSearchDocs
                  ? mxUtils.bind(this, function (searchStr, searchReadyCallback) {
                      this.remoteInvoke(
                        'searchDiagrams',
                        [searchStr, null],
                        null,
                ...
                mxUtils.bind(this, function (url, info, name) {
                  ...
                  parent.postMessage(
                    JSON.stringify({ event: 'template', docUrl: url, info: info, name: name }),
                    '*'
                  )
                }),
                null,
              ...
              parent.postMessage(
                JSON.stringify({ event: 'textContent', data: allPagesTxt, message: data }),
                '*'
              )
              return
              ...
              if (data.format == 'png' || data.format == 'xmlpng') {
                    ...
                    var req = new mxXmlRequest(
                      ...
                      'format=png&embedXml=' +
                        ...
                        (data.layerIds != null && data.layerIds.length > 0
                          ? '&extras=' +
                            encodeURIComponent(JSON.stringify({ layerIds: data.layerIds }))
                          : '') +
                ...
                var graphReady = mxUtils.bind(this, function () {
                  ...
                  if (
                    data.format == 'html2' ||
                    (data.format == 'html' &&
                      (urlParams['pages'] != '0' || (this.pages != null && this.pages.length > 1)))
                  ) {
                    ...
                    var bg =
                      data.background != null ? data.background : this.editor.graph.background
              ...
              this.hideDialog()
              console.log(autosave, 'eeeeeeeeeeeeeee')
              ...
              if (data.rect != null) {
                ...
                afterLoad = mxUtils.bind(this, function () {
                  ...
                  var h = Math.min(
                    this.minInlineHeight != null ? Math.max(this.minInlineHeight, h0) : h0,
                    ds.height - 80
                  )
                  ...
                  window.setTimeout(
                    mxUtils.bind(this, function () {
                      ...
                      if (
                        ...
                        this.formatWindow.window.div.style.top = graph.container.style.top
                      }
                      ...
                      if (graph.container.clientWidth > bounds.width + 2 * border) {
                        graph.container.scrollLeft =
                          bounds.x + (bounds.width + border - graph.container.clientWidth) / 2
                      }
                    }),
                    10
                  )
              ...
              if (file != null) {
                ...
                if (tmp != null && tmp != '') {
                  file.mergeFile(
                    ...
                    function (err) {
                      parent.postMessage(
                        JSON.stringify({ event: 'merge', message: data, error: err }),
                        '*'
                      )
                    }
                  )
              ...
              parent.postMessage(
                JSON.stringify({ error: 'unknownMessage', data: JSON.stringify(data) }),
                '*'
              )
  ...
  EditorUi.prototype.createEmbedButton = function (title, fn, shortcut, primary) {
    ...
    btn.setAttribute('title', title + (shortcut != null ? ' (' + shortcut + ')' : ''))
    btn.style.marginLeft = '6px'
  ...
  EditorUi.prototype.addEmbedButtons = function () {
    ...
    if (urlParams['noExitBtn'] != '1') {
      div.appendChild(
        this.createEmbedButton(
          urlParams['publishClose'] == '1' ? mxResources.get('close') : mxResources.get('exit'),
          this.actions.get('exit').funct
        )
      )
    }
      ...
      if (lines.length > 0) {
        ...
        try {
          for (var i = 0; i < arrays.length; i++) {
            ...
            var parent =
              parentIndex != null ? graph.model.getCell(namespace + values[parentIndex]) : null
            ...
            if (cell.vertex) {
              ...
              if (widthValue != null && widthValue != 'auto') {
                ...
                cell.geometry.width =
                  width == 'auto' || widthValue == 'auto' ? size.width + padding : parseFloat(width)
              }

              var heightValue =
                height.charAt(0) == '@' ? cell.getAttribute(height.substring(1)) : null
          ...
          for (var i = 0; i < parents.length; i++) {
            var widthValue =
              width.charAt(0) == '@' ? parents[i].getAttribute(width.substring(1)) : null
            var heightValue =
              height.charAt(0) == '@' ? parents[i].getAttribute(height.substring(1)) : null
                  ...
                  for (var j = 0; j < refs.length; j++) {
                    ...
                    if (ref != null) {
                      ...
                      if (edge.sourcelabel != null) {
                        label =
                          graph.replacePlaceholders(dataCell, edge.sourcelabel, vars) +
                          (label || '')
                      }
                      ...
                      if (edge.targetlabel != null) {
                        label =
                          (label || '') + graph.replacePlaceholders(ref, edge.targetlabel, vars)
                      }

                      var placeholders =
                        (edge.placeholders == 'target') == !edge.invert ? ref : realCell
                      var edgeStyle =
                        edge.style != null
                      ...
                      if (edge.labels != null) {
                        for (var k = 0; k < edge.labels.length; k++) {
                          ...
                          var el = new mxCell(def.label || k, new mxGeometry(elx, ely, 0, 0), st)
                          el.vertex = true
                          ...
                          if (def.placeholders != null) {
                            el.value = graph.replacePlaceholders(
                              (def.placeholders == 'target') == !edge.invert ? ref : realCell,
                              el.value,
                              vars
                            )
                          }
          ...
          if (cells.length > 0) {
            ...
            if (layout.charAt(0) == '[') {
              ...
              var orgChartLayout = new mxOrgChartLayout(graph, 2, levelspacing, nodespacing)
  ...
  EditorUi.prototype.updateUi = function () {
    ...
    var file = this.getCurrentFile()
    var active = file != null || (urlParams['embed'] == '1' && this.editor.graph.isEnabled())
    ...
    if (temp != null) {
      for (var i = 0; i < temp.length; i++) {
        this.libraryLoaded(
          ...
          temp[i].images,
          temp[i].title,
          temp[i].div != null ? temp[i].div.style.display != 'none' : temp[i].expand
        )
      }
    }
  }
  ...
  EditorUi.prototype.updateActionStates = function () {
    ...
    this.actions
      .get('autosave')
      .setEnabled(file != null && file.isEditable() && file.isAutosaveOptional())
    ...
    this.actions
      .get('rename')
      .setEnabled((file != null && file.isRenamable()) || urlParams['embed'] == '1')
    ...
    this.actions
      .get('editShape')
      .setEnabled(active && state != null && state.shape != null && state.shape.stencil != null)
  }
  ...
  EditorUi.prototype.showRemotelyStoredLibrary = function (title) {
    ...
    libsSection.innerHTML =
      '<div style="text-align:center;padding:8px;"><img src="' + IMAGE_PATH + '/spin.gif"></div>'
    ...
    this.remoteInvoke(
      ...
      mxUtils.bind(this, function (e) {
        ...
        mxUtils.write(
          status,
          e != null && e.message != null ? e.message : mxResources.get('unknownError')
        )
                  ...
                  mxUtils.bind(this, function (value) {
                    ...
                    var markAsMigrated = mxUtils.bind(this, function () {
                      ...
                      drawioFrame.contentWindow.postMessage(
                        JSON.stringify({ action: 'remoteInvoke', funtionName: 'setMigratedFlag' }),
                        '*'
                      )
                    })
                      ...
                      try {
                        ...
                        StorageFile.getFileContent(
                          ...
                          mxUtils.bind(this, function (data) {
                            if (
                              data == null ||
                              (fileTitle == '.scratchpad' && data == this.emptyLibraryXml)
                            ) {
                              //Don't overwrite
                              drawioFrame.contentWindow.postMessage(
                                JSON.stringify({
        ...
        try {
          ...
          for (var i = 0; i < storeName.length; i++) {
            trx
              ...
              .put(key != null && key[i] != null ? { key: key[i], data: data[i] } : data[i])
          }
        } catch (e) {
          if (error != null) {
            error(e)
  ...
  EditorUi.prototype.isRevisionHistoryEnabled = function () {
    ...
    return (
      file != null &&
      ((file.constructor == DriveFile && file.isEditable()) || file.constructor == DropboxFile)
    )
  }
...
var CommentsWindow = function (editorUi, x, y, w, h, saveCallback) {
  ...
  listDiv.style.backgroundColor = Editor.isDarkMode() ? Editor.darkColor : 'whiteSmoke'
  ...
  function writeCommentDate(comment, dateDiv) {
    ...
    dateDiv.setAttribute('title', ts.toLocaleDateString() + ' ' + ts.toLocaleTimeString())
  }
              ...
              function (markedOnly) {
                if (markedOnly === true) {
                  ...
                  noComments.style.display =
                    listDiv.getElementsByTagName('div').length == 0 ? 'block' : 'none'
                }
              },
              function (err) {
                showError(cdiv)
    ...
    if (!readOnly && !comment.isLocked && level == 0) {
      ...
      function toggleResolve(evt) {
        function doToggle() {
          ...
          mxUtils.write(
            resolveActionLnk,
            comment.isResolved ? mxResources.get('reopen') : mxResources.get('resolve')
          )
  ...
  if (editorUi.commentsSaveNeeded()) {
    ...
    saveLink.innerHTML =
      '<img src="' + IMAGE_PATH + '/save.png" style="width: 20px; padding: 2px;">'
      ...
      editorUi.getComments(
        ...
        mxUtils.bind(this, function (err) {
          listDiv.innerHTML = mxUtils.htmlEntities(
            mxResources.get('error') + (err && err.message ? ': ' + err.message : '')
          )
          this.hasError = true
        })
      )
