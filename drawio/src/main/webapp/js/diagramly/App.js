...
App = function (editor, container, lightbox) {
  EditorUi.call(
    ...
    lightbox != null
      ? lightbox
      : urlParams['lightbox'] == '1' || (uiTheme == 'min' && urlParams['chrome'] != '0')
  )
      ...
      if (file != null && file.isModified()) {
        var evt = {
          ...
          action:
            ...
            '-open_' +
            (file.opened != null ? Math.round((Date.now() - file.opened.getTime()) / 1000) : 'x') +
            '-save_' +
            (file.lastSaved != null
  ...
  this.editor.addListener(
    ...
    mxUtils.bind(this, function () {
      ...
      if (file != null) {
        EditorUi.logEvent({
          category: (this.editor.autosave ? 'ON' : 'OFF') + '-AUTOSAVE-FILE-' + file.getHash(),
          action: 'changed',
        ...
        if (typeof window.DriveClient === 'function') {
          if (
            ...
            if (
              ...
              (window.location.hash != null && window.location.hash.substring(0, 2) == '#G')
            ) {
        ...
        if (typeof window.DropboxClient === 'function') {
          if (
            ...
            if (
              ...
              (window.location.hash != null && window.location.hash.substring(0, 2) == '#D')
            ) {
        ...
        if (typeof window.TrelloClient === 'function') {
          if (
            ...
            if (
              ...
              (window.location.hash != null && window.location.hash.substring(0, 2) == '#T')
            ) {
  ...
  try {
    ...
    if (
      ...
      document.body.innerHTML =
        '<div style="margin-top:10%;text-align:center;">Stand-alone mode not allowed for this domain.</div>'
    ...
    if (window.mxscript != null) {
      ...
      if (
        ...
        (urlParams['embed'] != '1' || urlParams['embedRT'] == '1') &&
        urlParams['local'] != '1' &&
    ...
    function doLoad(bundle) {
      ...
      mxUtils.getAll(
        urlParams['dev'] != '1' ? [bundle] : [bundle, STYLE_PATH + '/default.xml'],
        function (xhr) {
          ...
          if (
            ...
            try {
              ...
              if (value != null) {
                ...
                if (
                  confirm(mxResources.get('configLinkWarn')) &&
            ...
            try {
              ...
              if (window.mxscript != null) {
                ...
                // Disables client
                  ...
                  typeof window.Dropbox.choose === 'undefined'
                ) {
                  window.DropboxClient = null
                }
                ...
                if (
                  ...
                  typeof OneDrive === 'undefined' &&
                  window.DrawOneDriveClientCallback != null &&
                ...
                if (
                  ...
                  !mxClient.IS_IE11 &&
                  typeof window.Trello === 'undefined' &&
                  window.DrawTrelloClientCallback != null &&
                  urlParams['tr'] == '1' &&
                  (navigator.userAgent == null ||
                    navigator.userAgent.indexOf('MSIE') < 0 ||
                    document.documentMode >= 10)
      ...
      try {
        ...
        if (Menus.prototype.defaultFonts != null) {
          for (var i = 0; i < Menus.prototype.defaultFonts.length; i++) {
            ...
            if (typeof value !== 'string' && value.fontFamily != null && value.fontUrl != null) {
...
App.prototype.init = function () {
   ...
   */
  this.gitHub =
    (!mxClient.IS_IE || document.documentMode == 10 || mxClient.IS_IE11 || mxClient.IS_EDGE) &&
    urlParams['gh'] != '0' &&
  ...
  try {
    if (
      ...
      if (hostname.endsWith('embed.diagrams.net') && window.location.ancestorOrigins.length > 0) {
        var img = new Image()
        img.src =
          'https://log.diagrams.net/images/1x1.png?src=EditorEmbedAncestorFrames' +
          '&v=' +
...
App.prototype.scheduleSanityCheck = function () {
  if (!mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp && this.sanityCheckThread == null) {
  ...
  if (file != null && file.isModified() && file.isAutosave() && file.isOverdue()) {
    var evt = {
      ...
      action:
        ...
        (file.saveLevel != null ? '-sl_' + file.saveLevel : '') +
        '-age_' +
        (file.ageStart != null ? Math.round((Date.now() - file.ageStart.getTime()) / 1000) : 'x') +
        (this.editor.autosave ? '' : '-nosave') +
        (file.isAutosave() ? '' : '-noauto') +
        '-open_' +
        (file.opened != null ? Math.round((Date.now() - file.opened.getTime()) / 1000) : 'x') +
    ...
    this.showError(
      ...
      mxUtils.bind(this, function () {
        ...
        this.actions.get(this.mode == null || !file.isEditable() ? 'saveAs' : 'save').funct()
      }),
      null,
      null,
      360,
...
App.prototype.appIconClicked = function (evt) {
  var file = this.getCurrentFile()
  var mode = file != null ? file.getMode() : null
  var url = file != null ? (mxEvent.isAltDown(evt) ? file.getFolderUrl() : file.getFileUrl()) : null
...
App.prototype.showSplash = function (force) {
  ...
  var showSecondDialog = mxUtils.bind(this, function () {
    ...
    this.showDialog(
      dlg.container,
      340,
      mxClient.IS_CHROMEAPP || EditorUi.isElectronApp ? 200 : 230,
      true,
      true,
      mxUtils.bind(this, function (cancel) {
        ...
        if (cancel && !mxClient.IS_CHROMEAPP) {
          var prev = Editor.useLocalStorage
          this.createFile(
            this.defaultFilename + (EditorUi.isElectronApp ? '.drawio' : ''),
            null,
            null,
            null,
            null,
        ...
        window.openFile.setConsumer(
          mxUtils.bind(this, function (xml, filename) {
            var doOpenFile = mxUtils.bind(this, function () {
              // Replaces PNG with XML extension
              var dot =
                !this.useCanvasForExport && filename.substring(filename.length - 4) == '.png'
            ...
            if (prev == null && mode == App.MODE_DEVICE) {
              ...
              var createFile = mxUtils.bind(this, function (folderId) {
                ...
                this.createFile(
                  name,
                  this.getFileData(
                    /(\.xml)$/i.test(name) || name.indexOf('.') < 0 || /(\.drawio)$/i.test(name),
                    /(\.svg)$/i.test(name),
                    /(\.html)$/i.test(name)
                  ),
                  null,
...
App.prototype.loadTemplate = function (url, onload, onerror, templateFilename, asLibrary) {
  ...
  if (!this.editor.isCorsEnabledForUrl(realUrl)) {
    ...
    realUrl =
      PROXY_URL + '?url=' + encodeURIComponent(url) + '&' + nocache + (base64 ? '&base64=1' : '')
  }
    ...
    mxUtils.bind(this, function (responseData) {
      try {
        ...
        if (isVisioFilename || this.isVisioData(data)) {
          ...
          this.importVisio(
            this.base64ToBlob(responseData.substring(responseData.indexOf(',') + 1)),
            function (xml) {
              onload(xml)
            },
            onerror,
...
App.prototype.fileCreated = function (file, libs, replace, done, clibs, success) {
  ...
  if (this.spinner.spin(document.body, mxResources.get('inserting'))) {
    ...
    var dataNode =
      data.length > 0
        ? this.editor.extractGraphModel(mxUtils.parseXml(data).documentElement, true)
        : null
          ...
          mxUtils.bind(this, function (text) {
            ...
            if (text != null && text.length > 0) {
              ...
              var tempFile = new LocalFile(
                this,
                text,
                urlParams['title'] != null ? decodeURIComponent(urlParams['title']) : filename,
                true
              )
              tempFile.getHash = function () {
                return id
          ...
          peer.getFile(
            ...
            mxUtils.bind(this, function (file) {
              ...
              if (currentFile == null) {
                ...
                this.editor.setStatus(
                  '<div title="' + status + '" class="geStatusAlert">' + status + '</div>'
                )
              }
  ...
  if (this.buttonContainer != null) {
    ...
    if (
      ...
      if (
        ...
        this.buttonContainer.style.display = this.buttonContainer.clientWidth == 0 ? 'none' : ''
      }
    }
          ...
          try {
            ...
            if (isLocalStorage) {
              localStorage.setItem(
                cachedNotifKey,
                JSON.stringify({ ts: Date.now(), notifs: notifs })
              )
            }
    ...
    mxEvent.addListener(
      ...
      mxUtils.bind(this, function (evt) {
        ...
        this.fullscreenElement.style.backgroundImage =
          "url('" +
          (this.fullscreenMode ? Editor.fullscreenExitImage : Editor.fullscreenImage) +
          "')"
...
App.prototype.updateUserElementStyle = function () {
  ...
  if (elt != null) {
    if (
      ...
      elt.style.height = ''
      elt.style.right =
        Editor.currentTheme == 'atlas' || this.darkModeElement != null ? '12px' : '26px'
        ...
        if (state != 1) {
          mxUtils.write(
            ...
            mxResources.get('realtimeCollaboration') +
              ': ' +
              (err != null && err.message != null ? err.message : mxResources.get('disconnected'))
          )
          this.userPanel.appendChild(div)
        }
      }
