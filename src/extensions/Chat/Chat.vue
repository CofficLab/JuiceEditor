<template>
  <NodeViewWrapper>
    <div
      class="chat"
      :class="node.attrs.position == 'start' ? 'chat-start  flex items-end' : 'chat-end'"
    >
      <!-- 对话框 -->
      <div class="chat-image flex flex-col items-end" contenteditable="false">
        ...
        <div class="dropdown dropdown-hover">
          ...
          <ul tabindex="0" class="chat-operators dropdown-content rounded-box" v-if="editable">
            <li class="cursor-pointer list-none hover:bg-base-200" @click="switchPosition">
              左右切换
            </li>
      ...
      <div
        ...
        :class="node.attrs.position == 'end' ? 'chat-bubble-info' : ''"
      >
        <NodeViewContent
          class="border border-dashed border-yellow-500/50 px-4 dark:border-cyan-800"
