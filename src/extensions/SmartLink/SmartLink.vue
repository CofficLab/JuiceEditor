	...
	<node-view-wrapper>
		<div class="dropdown dropdown-top dropdown-hover">
			<label tabindex="0" contenteditable="true" class="m-0 link link-hover link-primary">
				<p v-html="props.node.attrs.text"></p>
			...
			<div tabindex="0" class="dropdown-content z-50 menu p-2 w-96 flex flex-col gap-2">
				...
				<input type="text" v-model="props.node.attrs.href" placeholder="Search"
					class="input focus:outline-none input-primary w-full bg-opacity-90" />
				<input type="text" v-model="props.node.attrs.text" placeholder="Search"
					class="input focus:outline-none input-primary w-full bg-opacity-90" />
			</div>
		</div>
	</node-view-wrapper>
...
const notLink = () => {
	console.log(props.node.content.size)
	console.log(props.getPos())
	props.editor.chain().setNodeSelection(props.getPos()).toggleLink().run()
}

watch(() => props.node.attrs.href, () => {
	props.updateAttributes({
		href: props.node.attrs.href
	})
})

watch(() => props.node.attrs.text, () => {
	props.updateAttributes({
