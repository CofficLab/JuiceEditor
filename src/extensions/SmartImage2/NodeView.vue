<template>
    <node-view-wrapper ref="containerRef" class="umo-node-view" :id="node.attrs.id" :style="nodeStyle"
        @dblclick="imagePreview = node.attrs.src">
        <div class="umo-node-container umo-hover-shadow umo-select-outline umo-node-image" :class="{
            ...
            <div v-if="node.attrs.src && isLoading" class="loading" :style="{ height: `${node.attrs.height}px` }">
                <SmartIcon name="loading" class="loading-icon" />
                {{ t('node.image.loading') }}
            </div>
            <div class="error" v-else-if="node.attrs.src && error" :style="{ height: `${node.attrs.height}px` }">
                <SmartIcon name="image-failed" class="error-icon" />
                {{ t('node.image.error') }}
            </div>
            <drager v-else :selected="selected" :rotatable="true" :boundary="false"
                :draggable="Boolean(node.attrs.draggable) && !options.document.readOnly" :angle="node.attrs.angle"
                :width="Number(node.attrs.width)" :height="Number(node.attrs.height)" :left="Number(node.attrs.left)"
                :top="Number(node.attrs.top)" :min-width="14" :min-height="14" :max-width="maxWidth"
                :max-height="node.attrs.equalProportion ? maxHeight : undefined" :z-index="10"
                :equal-proportion="node.attrs.equalProportion" @rotate="onRotate" @resize="onResize"
                @resizeStart="onResizeStart" @resizeEnd="onResizeEnd" @drag="onDrag" @click="selected = true">
...
import SmartIcon from '../../ui/SmartIcon.vue'
import { computed, nextTick, ref, watch } from 'vue';

const { node, getPos, updateAttributes, editor } = defineProps(nodeViewProps)
