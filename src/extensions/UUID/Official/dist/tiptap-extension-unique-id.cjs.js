...
var t = require('@tiptap/core'),
    e = require('@tiptap/pm/model'),
...
const i = t.TiptapExtension.create({
    name: 'uniqueID',
    priority: 1e4,
    addOptions: () => ({
        attributeName: 'id',
    ...
    onCreate() {
        ...
        const { view: e, state: n } = this.editor,
            { tr: r, doc: o } = n,
    ...
    addProseMirrorPlugins() {
        ...
        return [
            new n.Plugin({
                key: new n.PluginKey('uniqueID'),
                appendTransaction: (e, n, r) => {
                    const i = e.some((t) => t.docChanged) && !n.doc.eq(r.doc),
                        a =
                        ...
                        s = e.find((t) => t.getMeta('y-sync$'))
                    if (s) return
                    if (!i || a) return
                    const { tr: d } = r,
                        { types: u, attributeName: p, generateID: l } = this.options,
                        c = t.combineTransactionSteps(n.doc, e),
                        { mapping: f } = c
                    return (
                        t.getChangedRanges(c).forEach(({ newRange: e }) => {
                            const n = t.findChildrenInRange(r.doc, e, (t) =>
                                    u.includes(t.type.name)
                                ),
                                i = n.map(({ node: t }) => t.attrs[p]).filter((t) => null !== t)
                            n.forEach(({ node: t, pos: e }, r) => {
                                ...
                                const s =
                                    null === (a = d.doc.nodeAt(e)) || void 0 === a
                                        ...
                                        : a.attrs[p]
                                if (null === s)
                                    return void d.setNodeMarkup(e, void 0, { ...t.attrs, [p]: l() })
                                const u = n[r + 1]
                    ...
                    transformPasted: (t) => {
                        ...
                        const { types: n, attributeName: r } = this.options,
                            o = (t) => {
                                ...
                                return (
                                    t.forEach((t) => {
                                        if (t.isText) return void i.push(t)
                                        if (!n.includes(t.type.name))
                                            return void i.push(t.copy(o(t.content)))
                                        const e = t.type.create(
                                            { ...t.attrs, [r]: null },
                                            o(t.content),
                                            t.marks
                                        )
                        ...
                        return (i = !1), new e.Slice(o(t.content), t.openStart, t.openEnd)
                    }
                }
            })
        ]
...
;(exports.UniqueID = i), (exports.default = i)
